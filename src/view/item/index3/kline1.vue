...
export default {
  ...
  created() {
    ...
    // let cpi_data = getAjax(url_name.cpi).responseJSON.result.data;
    // let ppi_data = getAjax(url_name.ppi).responseJSON.result.data;
    // let cpi_time = this.dealData5(cpi_data).time.reverse();
    // let ppi_time = this.dealData6(ppi_data).time.reverse();
    console.log("658", getAjax(url_name.liliFun("658", 1)));
    let lili_data = JSON.parse(getAjax(url_name.liliFun("658", 1)).responseText)
      .result.data;

    let lili_data2 = JSON.parse(
      getAjax(url_name.liliFun("658", 2)).responseText
    ).result.data;
    let lili_data3 = JSON.parse(
      getAjax(url_name.liliFun("658", 3)).responseText
    ).result.data;
    let lili_data4 = JSON.parse(
      getAjax(url_name.liliFun("658", 4)).responseText
    ).result.data;
    ...
    lili_all = lili_all.concat(that.dealData4(lili_data3, 2).cn);
    lili_all = lili_all.concat(that.dealData4(lili_data4, 2).cn);
    ...
    // let url_rmb =
    //   "http://hq.cnindex.com.cn/market/market/getIndexDailyData?indexCode=CNYX&startDate=2016-03-16&endDate=" +
    ...
    let array4 = [
      array2,
      // {
      //   num: hushen300_pe.pe.slice(-50),
      //   name: "沪深300的pe值—基金",
      //   time: hushen300_pe.time.slice(-50),
      // },
      {
        name: "美元离岸人民币",
    ...
    var warnLine = 7.0001;
    let array3_num = array3.num.slice(-array2.time.length);

    var maxVal = Math.max.apply(null, array3_num);
    var minVal = parseInt(Math.min.apply(null, array3_num));

    var warnProp = 1 - (maxVal - warnLine) / (maxVal - minVal);
      ...
      {},
      {
        color: {
          ...
          colorStops: [
            {
              offset: 0,
              color: "#e7224350", // 0% 处的颜色
            },
            {
              offset: warnProp,
              color: "#e7224350", // 标线处的颜色
            },
            {
              offset: warnProp,
              color: "#26D4AD50", // 标线处的颜色
            },
            {
              offset: 1,
              color: "#26D4AD50", // 100% 处的颜色
            },
          ],
  ...
  methods: {
    url1(num1, num2) {
      return (
        ...
        num1 +
        "&fields1=f1,f2,f3,f4,f5&fields2=f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61&lmt=1678&klt=101&fqt=1&end=30000101"
      );
    },
    dateFormat(fmt, date) {
      let ret;
    ...
    dealData2(data2, type, name, num) {
      ...
      data2.forEach((element2, index2) => {
        ...
        num1.push(parseFloat(element2_item[num]));
      });
