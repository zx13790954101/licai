      ...
      if (item != max_num_level30[index - 1]) {
        ...
        if (min_num_level30[index] == min_num_level30[index - 1]) {
          if (min_num_level30[index - 1] < min_num_level30[index - 2]) {
            if (
              min_num_level30[index - 1] - min_num_level30[index - 2] < 0
            )
              if (max_num_level30[index - 1] == max_num_level30[index - 2]) {
...
export const dealSeatType = (data) => {
  ...
  let arr_index = return_array.findIndex(
...
function judgesSeat (array_data, this_data) {
  let arr_index = array_data.findIndex(
    (item) => item.active === "active"
  )
  if (this_data[this_data.length - 1] - this_data[this_data.length - 3] >
    0) {
    if (arr_index != 0) {
      if (arr_index > 0) {
...
function seatTtpeItem (data, this_data, time) {
  ...
  let return_value
  if (
    parseFloat(data[length_data - 2]) >=
    parseFloat(this_data[length_data - 2])
  ) {
    return_value =
  ...
  let number =
    (data[length_data - 1] - this_data[length_data - 1]) / this_data[length_data - 1]
  return {
